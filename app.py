...
以下のルールを厳格に守ること。
【ルール】
・冒頭の1文で"なぜ御社に連絡したのか"の理由を具体的に書く（例：導入事例で〇〇という数字を出されていたので〜等）
・相手の事業内容に対して、自分がどう貢献できるかを1〜2行で書く
・自社サービスの説明は3行以内（SEO、X運用、リード獲得支援などを想定して適当な強みを設定してください）
・「突然のご連絡失礼いたします」「貴社のご発展に〜」などのテンプレ感のある定型句は一切使わない
・全体を200字以内に収める
・最後は「ご興味があればお返事ください」のシンプルなクロージングにする
