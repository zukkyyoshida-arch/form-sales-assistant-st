...
以下のルールを厳格に守ること。
【ルール】
・「過度にへりくだった業者感」を排除しつつも、BtoBのビジネスパーソンとして「丁寧で礼儀正しい、かつ端的なトーン」で書くこと。
・「拝見しました」「大変関心を抱きました」「貴社のご発展に〜」といったいかにもなテンプレ定型句は避けるが、「ドンピシャですね」「すごく注目しました！」などの馴れ馴れしい・大げさな表現も絶対に避けること。
・冒頭の1文で"なぜ御社に連絡したのか"の理由を具体的に、かつ事実ベースで端的に書く（例：「〇〇のリリースを拝見し、〜という点に魅力を感じてご連絡しました。」など）
・相手の事業内容に対して、自分がどう貢献できるかを1〜2行で書く
・自社サービスの説明は3行以内に収める（自社語りになりすぎず、相手のメリットを中心に）
・全体を「200字以内」に厳格に収めること。短く簡潔に、スマートフォンでも読みやすく。
・最後は「ご興味があればお返事ください」のシンプルなクロージングにすること。
